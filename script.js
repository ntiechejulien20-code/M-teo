...
function locateAndLoad(){
  ...
  // L'API navigator.geolocation exige un "contexte sécurisé" (HTTPS ou
  // localhost/127.0.0.1). Sur file:// ou un http:// distant, la plupart
  // des navigateurs bloquent l'accès -> on prévient clairement au lieu
  // de tomber sur Paris sans explication.
  if(!window.isSecureContext){
    els.status.textContent =
      "La géolocalisation nécessite HTTPS (ou localhost). Ouvre ce site via un serveur local ou en HTTPS — affichage de Paris par défaut.";
    ...
    (err) => {
      ...
      switch(err.code){
        case err.PERMISSION_DENIED:
          reason = "Permission de localisation refusée. Autorise l'accès à la position dans les réglages du navigateur (et de l'appareil).";
          break;
        case err.POSITION_UNAVAILABLE:
          reason = "Position indisponible pour le moment (GPS/réseau).";
          break;
        case err.TIMEOUT:
          reason = "La localisation a mis trop de temps à répondre.";
          break;
        default:
          reason = "Erreur de localisation inconnue.";
      }
      els.status.textContent = `${reason} Affichage de Paris par défaut.`;
